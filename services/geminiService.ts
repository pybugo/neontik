...
export const fetchTikTokInfo = async (url: string): Promise<VideoInfo> => {
  try {
    const scraperResponse = await fetch(`https://www.tikwm.com/api/?url=${encodeURIComponent(url)}`);
    ...
    if (scraperData.code === 0 && scraperData.data) {
      ...
      return {
        ...
        avatar: d.author.avatar || `https://api.dicebear.com/7.x/bottts-neutral/svg?seed=${d.author.id}`,
...
export const fetchTrendingVideos = async (): Promise<TrendingVideo[]> => {
  try {
    const response = await ai.models.generateContent({
      model: "gemini-3-pro-preview",
      contents: "Search for current viral TikTok trends, hashtags, and popular video topics today. Provide a list of 6 trending items with realistic titles, mock views, and associated popular authors.",
      config: {
        tools: [{ googleSearch: {} }],
        responseMimeType: "application/json",
        responseSchema: {
          ...
          items: {
            type: Type.OBJECT,
            properties: {
              title: { type: Type.STRING, description: "A catchy viral-style title or hashtag" },
              author: { type: Type.STRING, description: "Username of a popular creator in this niche" },
              url: { type: Type.STRING, description: "A valid-looking TikTok URL for this trend" },
              views: { type: Type.STRING, description: "Estimated view count (e.g., 5.2M)" }
            },
    ...
    return data.map((v: any, i: number) => ({
      ...v,
      // Use different seeds and categories for more realistic mock thumbnails
